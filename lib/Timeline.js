'use strict';

var request = require('request');

var Pin = require('./Pin');

var API_ROOT = 'https://timeline-api.getpebble.com';

/**
 * Timeline
 *
 * @param {Object} `opts` [optional]
 */
var Timeline = module.exports = function (opts) {
  opts = opts || {};

  this._apiKey;
  this._userKey;
  this._topics = [];

  this._apiRoot = opts.apiRoot || process.env.TIMELINE_API_ROOT || API_ROOT;
};

Timeline.Pin = Pin;

/**
 * Set the API Key
 *
 * @param {String} `key`
 */
Timeline.prototype.setApiKey = function (key) {
  this._apiKey = key;
};

/**
 * Set the User Key
 *
 * @param {String} `key`
 */
Timeline.prototype.setUserKey = function (key) {
  this._userKey = key;
};

/**
 * Set the Topics
 *
 * @param {Array} `topics`
 */
Timeline.prototype.setTopics = function (topics) {
  this._topics = topics;
};

/**
 * Add a Topic
 *
 * @param {String} `topic`
 */
Timeline.prototype.addTopic = function (topic) {
  this._topics.push(topic);
};

/**
 * Set the API Root
 *
 * @param {String} `url`
 */
Timeline.prototype.setApiRoot = function (url) {
  this._apiRoot = url;
};

/**
 * Send a pin to a user
 *
 * @param {Pin} `pin`
 */
Timeline.prototype.sendUserPin = function (pin) {
  if (!this._userKey) {
    console.log('Error: user key not set');
    return;
  }

  if (!pin instanceof(Pin)) {
    console.log('Error: invalid pin');
    return;
  }

  pin = pin.getPin();

  var headers = {'X-User-Token': this._userKey};

  this._put('/v1/user/pins/' + pin.id, headers, pin, function (err, data, res) {
    console.log(data);
  });
};

/**
 * Send a pin to a topic
 *
 * @param {Pin} `pin`
 */
Timeline.prototype.sendSharedPin = function (pin) {
  if (!this._apiKey) {
    console.log('Error: api key not set');
    return;
  }

  if (!pin instanceof(Pin)) {
    console.log('Error: invalid pin');
    return;
  }

  pin = pin.getPin();

  var headers = {
    'X-API-Key': this._apiKey,
    'X-PIN-Topics': this._topics.toString()
  };

  this._put('/v1/shared/pins/' + pin.id, headers, pin, function (err, body, res) {
    console.log(body);
  });
};

/**
 * Subscribe to a topic
 *
 * @param {String} `topic`
 */
Timeline.prototype.subscribe = function (topic) {
  if (!this._userKey) {
    console.log('Error: user key not set');
    return;
  }

  var headers = {'X-User-Token': this._userKey};

  this._post('/v1/user/subscriptions/' + topic, headers, null, function (err, body, res) {
    console.log(body);
  });
};

/**
 * Unsubscribe from a topic
 *
 * @param {String} `topic`
 */
Timeline.prototype.unsubscribe = function (topic) {
  if (!this._userKey) {
    console.log('Error: user key not set');
    return;
  }

  var headers = {'X-User-Token': this._userKey};

  this._delete('/v1/user/subscriptions/' + topic, headers, function (err, body, res) {
    console.log(body);
  });
};


Timeline.prototype._put = function(endpoint, headers, body, callback) {
  return this.__request('PUT', endpoint, headers, body, callback);
};

Timeline.prototype._post = function(endpoint, headers, body, callback) {
  return this.__request('POST', endpoint, headers, body, callback);
};

Timeline.prototype._delete = function(endpoint, headers, callback) {
  return this.__request('DELETE', endpoint, headers, null, callback);
};

Timeline.prototype.__request = function(method, endpoint, headers, body, callback) {
  var options = {};

  options.method = method.toUpperCase();
  options.headers = headers || {};
  options.url = this._apiRoot + endpoint;

  switch (method) {
    case 'PUT':
      options.body = body;
      options.json = true;
      break;
  }

  request(options, function (err, res, body) {
    if (err) {
      return callback(err, body, res);
    }

    switch (res.statusCode) {
      case 400:
        callback(new Error('The pin object submitted was invalid.'), body, res);
        break;
      case 403:
        callback(new Error('The API key submitted was invalid.'), body, res);
        break;
      case 410:
        callback(new Error('The user token submitted was invalid or does not exist.'), body, res);
        break;
      case 429:
        callback(new Error('Server is sending updates too quickly.'), body, res);
        break;
      case 523:
        callback(new Error('Could not save pin due to a temporary server error.'), body, res);
        break;
      case 200:
      default:
        callback(null, body, res);
        break;
    }
  });
};
