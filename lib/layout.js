...
module.exports = function (opts) {
  ...
  if (this.headings || this.paragraphs) {
    if (!this.headings) {
      throw new Error('headings is required if paragraphs is set');
    }
    if (!this.paragraphs) {
      throw new Error('paragraphs is required if headings is set');
    }
    if (this.headings.length !== this.paragraphs.length) {
      throw new Error('number is headings must equal the number of paragraphs');
    }
  }
